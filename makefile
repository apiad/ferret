...
.PHONY: docker-build
docker-build:
	docker build -t ferret-io:latest -f dockerfile .

.PHONY: clean
clean:
	rm -rf dist
	rm -rf ferret_io.egg-info
	rm -rf *.db*
...
release: format-check
	...
	@echo Replace version in ferret/__init__.py
	@sed -i.bak "s/__version__ = \"$(CURRENT_VERSION)\"/__version__ = \"$(NEW_VERSION)\"/" ferret/__init__.py

	@echo Remove backup files
	@rm pyproject.toml.bak ferret/__init__.py.bak

	@uv sync

	@echo "Committing version bump..."
	@git add pyproject.toml ferret/__init__.py uv.lock
